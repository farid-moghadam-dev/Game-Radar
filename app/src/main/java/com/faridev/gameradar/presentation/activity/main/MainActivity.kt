...
import com.faridev.gameradar.presentation.feature.entity.detail.EntityDetailScreen
import com.faridev.gameradar.presentation.feature.entity.list.EntityListScreen
        ...
        setContent {
            GameRadarTheme {
                ...
                Scaffold(modifier = Modifier.fillMaxSize()) { innerPadding ->
                    MainNavigationDrawer(modifier = Modifier.padding(innerPadding))
                }
            }
        }
    }
...
private fun MainNavigationDrawerContent(
    ...
    Column {
        ...
        LazyColumn(modifier = Modifier.padding(top = 10.dp)) {
            items(GameEntityType.entries.toList()) { type ->
                DrawerItem(type.displayName) { onItemClick(type) }
            }
        }
    }
}
...
private fun MainScreenContent(navController: NavHostController) {
    Scaffold(modifier = Modifier.fillMaxSize()) { _ ->
        NavHost(
            ...
            composable<NavRoutes.EntityList> { backStackEntry ->
                val route = backStackEntry.toRoute<NavRoutes.EntityList>()
                EntityListScreen(type = route.type) { type, entityId ->
                    navController.navigate(NavRoutes.EntityDetail(type, entityId))
                }
            }
            composable<NavRoutes.EntityDetail> { backStackEntry ->
                val route = backStackEntry.toRoute<NavRoutes.EntityDetail>()
                EntityDetailScreen(
                    type = route.type,
